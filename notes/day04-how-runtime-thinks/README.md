...
Day04 不再只按“天”推进，而是拆成多个 Part。Part A 先建立世界观：Prompt 不是 Context，Context 是 Runtime State 的一次 Projection。Part D 完成 Context Window Management，回答有限上下文空间中的工业级取舍问题。Part E 完成 Provider Adapter，补齐 Context Builder 之后的模型供应商适配层，并为 Day05 Tool Calling 做好边界准备。
...
- Day04 Part E：Provider Adapter
  - [Markdown 主版本](day04-part-e-provider-adapter.md)
  - [PDF 阅读版](day04-part-e-provider-adapter.pdf)
  - [DOCX 可编辑版](day04-part-e-provider-adapter.docx)
  - [ChatGPT 分享会话源记录](source/day04-part-e-chatgpt-share-source.md)
...
- Provider Adapter 是 Runtime 和外部 LLM Provider 之间的边界层
- Context Builder 决定“给模型看什么”，Provider Adapter 决定“如何让某个模型收到这些内容”
- Runtime 不应该直接保存 OpenAI Message、Claude Message 或 Gemini Content，而应该定义自己的 RuntimeMessage、ContentBlock、ToolDefinition、ToolCall 和 RuntimeEvent
- Unified Model Interface 让 Runtime 依赖模型能力，而不是依赖某个 Provider SDK
- Provider Adapter 至少需要处理 Message Converter、Tool Converter、Stream Converter、Error Mapper、Usage Mapper 和 Response Converter
- Streaming Event、Tool Calling、Error Format 和 Usage Metric 都需要从 Provider 协议映射成 Runtime 内部统一语义
- LLM 负责产生 Tool Call 意图，真正执行 Tool 的是 Runtime
- Mock Provider 是测试 Runtime State、Context Builder、Tool Loop 和 State Transition 的基础设施
- Workspace Index 属于 Runtime State，是 Coding Agent 对工作空间的当前认知
- Day04 最终形成 Runtime State、Context Builder、Context Window Management、Runtime Model、Provider Adapter、Tool Runtime 的完整架构地图
